...
// Must match what the servers actually register — `server.mjs` and the remote
// `api/src/mcp/tools.ts` both filter on `!sdkOnly && sdk !== false`. Filtering on
// `!sdkOnly` alone documented 9 tools that neither surface exposes.
const exposed = api.methods.filter((m) => !m.sdkOnly && m.sdk !== false);
