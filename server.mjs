...
//
// Two passes, and the order matters: schemas cross-reference each other by
// *filename* (`"$ref": "domain.json"`) while each file's `$id` is the bare stem
// ("domain"), so a ref only resolves once every file is registered under its
// filename. Compiling inside the read loop threw MissingRefError on the first
// ref-bearing schema — whichever one readdirSync happened to reach first.
...
for (const id of Object.keys(schemas)) validators[id] = ajv.getSchema(`${id}.json`);
...
const tools = api.methods
  .filter((method) => !method.sdkOnly && method.sdk !== false)
  .map((method) => ({
